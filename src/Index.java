...
public class Index {
    ...
    public static void delete(File f) {
    	//check if it's directory or not? if yes we need to delete all things in it recursively
    	if(f.isDirectory()) {
	...
	public static int runIndexer(String method, String dataDirname, String outputDirname) throws IOException 
	{
		...
		for (File block : dirlist) {
			...
			Map<Integer, List<Integer>> postingLists = new TreeMap<Integer, List<Integer>>();
			
			for (Pair<Integer, Integer> item : tempTermDoc) {
				if(!postingLists.containsKey(item.getFirst())) {
					List<Integer> tempList = new ArrayList<Integer>();
					tempList.add(item.getSecond());
					postingLists.put(item.getFirst(), tempList);	
				} else {
